...
TODO:
- кэш (фронт, бэк)(оптимизированные запросы в бд(ИНДЕКСЫ), замерять с помощью того прикола expect analyse)
- ускорение диссерелизации объектов
- многопоточность и асинхронность
- попробовать jquery или jrpc Ради прикола(в следующем проекте)
- микросерверность (по идее уже соблюдается, забьем)
...
- логирование, а потом в графану
- тестирование
- celery
- обернуть в удобный докер контейнер+ пайплайн cicd и деплой в кубер(но чето сильно круто)
